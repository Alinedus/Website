...
export async function buildWordmark() {
  ...
  // (the `d` ascender is located from the beads themselves, below)
  ...
  //
  // Its column comes from the SAMPLED BEADS, not from prefix measurement.
  // `measureText` accumulates the tracking applied above, so a prefix width
  // drifts further from the rendered advance with every glyph: at the `i` the
  // error is invisible, and by the `d` it is a full letter — which parked the
  // film's protagonist over the `e`. The ascender is just the highest bead in
  // the right-hand part of the word, and no font metric can get that wrong.
  // The `d`'s ascender is the RIGHTMOST column of the word — unambiguous in
  // "alined", and independent of every font metric.
  let rightMost = -Infinity
  for (let i = 0; i < count; i++) {
    if (points[i * 3] > rightMost) rightMost = points[i * 3]
  }
  let dotX = 0
  let dotY = -Infinity
  for (let i = 0; i < count; i++) {
    if (points[i * 3] < rightMost - dotRadius * 2.5) continue
    if (points[i * 3 + 1] > dotY) {
      dotY = points[i * 3 + 1]
      dotX = points[i * 3]
    }
  }
